    ...
    async function loadGame() {
        try {
            ...
            if (!data.ok || !data.game) {
                throw new Error(data.error || 'ゲームが見つかりませんでした');
            }
    ...
    function renderGame(game) {
        ...
        if (metaListEl) {
            ...
            metaListEl.innerHTML = `
                ...
                <li>・プレイ人数: ${players}</li>
                <li>・プレイ時間: ${game.play_time || ''}</li>
                <li>・難易度: ${game.difficulty || ''}</li>
            `;
        }
    }

    function showNotFound(msg = 'ゲームが見つかりませんでした') {
        const container = document.querySelector('.game-details-card');
        if (container) {
            container.innerHTML = `<h2>${msg}</h2><a href="game.php">一覧に戻る</a>`;
        }
    }
